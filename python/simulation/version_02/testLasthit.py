...
#        mini_batch = random.sample(self.memory, len(self.memory))
        ...
        error = self.model.fit(update_input, target,
...
class LasthitEnvironment:
    ...
    def step(self, action):
        ...
        ###  hero
        damage_hero = random.randint(19,21)
        self.can_hit -= 0.1        
        ...
        if self.current_attack:
            round_loop += self.current_attack_time
            for round_l in range(round_loop):
                done = self.creep_attack()            
        else:
            done = self.creep_attack()
        ...
        if self.current_attack and not done:
